...
######################################################################
      ...
      all.presAbs_topoANDLiDAR.results = matrix(data=NA, ncol=3, nrow=1)
      rownames(all.presAbs_topoANDLiDAR.results) = colnames(presAbs.response.cgs)
      colnames(all.presAbs_topoANDLiDAR.results) = c("OOB", "absent", "present")
      
      #Run Random Forest
      presAbs_topoANDLiDAR_rf <- randomForest(y=as.factor(presAbs.response.cgs[run.sub, ]), 
                                              x=(all_predictors_complete[run.sub, ]))
      
      all.presAbs_topoANDLiDAR.results[1,"OOB"] = round(1-presAbs_topoANDLiDAR_rf$err.rate[500,1], digits=3)
      all.presAbs_topoANDLiDAR.results[1,"absent"] = round(1-presAbs_topoANDLiDAR_rf$err.rate[500,2], digits=3)
      all.presAbs_topoANDLiDAR.results[1,"present"] = round(1-presAbs_topoANDLiDAR_rf$err.rate[500,3], digits=3);
      all.presAbs_topoANDLiDAR.results
      ...
      #Set up results table
      all.presAbs_LiDAR.results = matrix(data=NA, ncol=3, nrow=1)
      rownames(all.presAbs_LiDAR.results) = colnames(presAbs.response.cgs)
      colnames(all.presAbs_LiDAR.results) = c("OOB", "absent", "present")
      
      #Run Random Forest
      presAbs_LiDAR_rf <- randomForest(y=as.factor(presAbs.response.cgs[run.sub, ]), x=(LiDAR_complete[run.sub,]))
      
      all.presAbs_LiDAR.results[1,"OOB"] = round(1-presAbs_LiDAR_rf$err.rate[500,1], digits=3)
      all.presAbs_LiDAR.results[1,"absent"] = round(1-presAbs_LiDAR_rf$err.rate[500,2], digits=3)
      all.presAbs_LiDAR.results[1,"present"] = round(1-presAbs_LiDAR_rf$err.rate[500,3], digits=3);
      all.presAbs_LiDAR.results
      ...
      #Set up results table
      all.presAbs_topo.results = matrix(data=NA, ncol=3, nrow=1)
      rownames(all.presAbs_topo.results) = colnames(presAbs.response.cgs)
      colnames(all.presAbs_topo.results) = c("OOB", "absent", "present")
      
      #Run Random Forest
      presAbs_topo_rf <- randomForest(y=as.factor(presAbs.response.cgs[run.sub, ]), x=(topo_complete[run.sub,]))
      
      all.presAbs_topo.results[1,"OOB"] = round(1-presAbs_topo_rf$err.rate[500,1], digits=3)
      all.presAbs_topo.results[1,"absent"] = round(1-presAbs_topo_rf$err.rate[500,2], digits=3)
      all.presAbs_topo.results[1,"present"] = round(1-presAbs_topo_rf$err.rate[500,3], digits=3);
      all.presAbs_topo.results
...
##########################################
      ...
      topo_presAbs_predicted <- as.integer(as.character(predict(object=presAbs_topo_rf, topo_complete)))
      topo_presAbs_err_matrix <- make_classError_matrix(NClasses=2, actual=presAbs.response.cgs, predicted=topo_presAbs_predicted)
      topo_class_error_presAbs <- class_error_presAbs(topo_presAbs_err_matrix)
      
      LiDAR_presAbs_predicted <- as.integer(as.character(predict(object=presAbs_LiDAR_rf, LiDAR_complete)))
      lidar_presAbs_err_matrix <- make_classError_matrix(NClasses=2, actual = presAbs.response.cgs, predicted = LiDAR_presAbs_predicted)
      lidar_class_error_presAbs <- class_error_presAbs(lidar_presAbs_err_matrix)
      
      topo_and_LiDAR_PresAbs_predicted <- as.integer(as.character(predict(object=presAbs_topoANDLiDAR_rf, all_predictors_complete)))
      lidarANDtopo_presAbs_err_matrix <- make_classError_matrix(NClasses=2, actual = presAbs.response.cgs, predicted = topo_and_LiDAR_PresAbs_predicted)
      lidarANDtopo_class_error_presAbs <- class_error_presAbs(lidarANDtopo_presAbs_err_matrix)
      ...
      only_present_shrub_locations <- actual.shrub.locations_raster
      only_present_shrub_locations[only_present_shrub_locations == 0] <- NA
      plot(only_present_shrub_locations, col = "RED")
      
      ##################################
      #### Topo predicted locations ####
      ##################################
      topo.predicted.allshrub.locations <- matrix(data = NA, nrow = length(all.complete))
      
      topo.predicted.allshrub.locations[all.complete] <- topo_presAbs_predicted
      
      topo.predicted.allshrub.locations_raster <- raster(topo.predicted.allshrub.locations, template = example_raster_2m)
      ...
      plot(topo.predicted.allshrub.locations_raster, main = "Topo All Shrub Predicted Locations", col =c("#D3D3D3", "#4daf4a"), legend = FALSE)
      plot(only_present_shrub_locations, col="#e41a1c", add= TRUE, legend = FALSE)
      ...
      plot(topo.predicted.allshrub.locations_raster + (2*actual.shrub.locations_raster), 
      ...
      ###################################
      LiDAR.predicted.allshrub.locations <- matrix(data = NA, nrow = length(all.complete))
      
      LiDAR.predicted.allshrub.locations[all.complete] <- LiDAR_presAbs_predicted
      
      LiDAR.predicted.allshrub.locations_raster <- raster(LiDAR.predicted.allshrub.locations, template = example_raster_2m)
      ...
      plot(LiDAR.predicted.allshrub.locations_raster, main = "LiDAR All Shrub Predicted Locations",  col =c("#D3D3D3", "#4daf4a"), legend = FALSE)
      plot(only_present_shrub_locations, col="#e41a1c", add= TRUE, legend = FALSE)
      legend("topright", legend = c("Predicted Shrub Absence", "Predicted Shrub Presence", "Actual Shrub Locations"), fill = c("#D3D3D3", "#4daf4a", "#e41a1c"))
      #dev.off()
      ...
      plot(LiDAR.predicted.allshrub.locations_raster + (2*actual.shrub.locations_raster), 
      ...
      ############################################
      topo_and_LiDAR.predicted.allshrub.locations <- matrix(data = NA, nrow = length(all.complete))
      
      topo_and_LiDAR.predicted.allshrub.locations[all.complete] <- topo_and_LiDAR_PresAbs_predicted
      
      topo_and_LiDAR.predicted.allshrub.locations_raster <- raster(topo_and_LiDAR.predicted.allshrub.locations, template = example_raster_2m)
      ...
      plot(topo_and_LiDAR.predicted.allshrub.locations_raster, main = "Topo and LiDAR All Shrub Predicted Locations", col =c("#D3D3D3", "#4daf4a"), legend = FALSE)
      plot(only_present_shrub_locations, col="#e41a1c", add= TRUE, legend = FALSE)
      legend("topright", legend = c("Predicted Shrub Absence", "Predicted Shrub Presence", "Actual Shrub Locations"), fill = c("#D3D3D3", "#4daf4a", "#e41a1c"))
      #dev.off()
      ...
      plot(topo_and_LiDAR.predicted.allshrub.locations_raster + (2*actual.shrub.locations_raster), 
...
#########################################################
output <- file("new_gridmetrics_parsimonious_output_topoandLiDAR_04_18_16.txt")
...
parsimonious_topoANDLiDAR_presAbs <- verifyParsimonious(predictors=all_predictors_complete, 
                                                        responses=presAbs.response.cgs, 
                                                        rf=presAbs_topoANDLiDAR_rf, 
                                                        subset=run.sub,
                                                        npredictors=10)
gc()
# Topo
parsimonious_topo_presAbs <- verifyParsimonious(predictors=topo_complete, 
                                                responses=presAbs.response.cgs, 
                                                rf=presAbs_topo_rf, 
                                                subset=run.sub,
                                                npredictors=10)
gc()
# LiDAR
parsimonious_LiDAR_presAbs <- verifyParsimonious(predictors= LiDAR_complete, 
                                                 responses=presAbs.response.cgs, 
                                                 rf=presAbs_LiDAR_rf, 
                                                 subset=run.sub,
                                                 npredictors=10)
